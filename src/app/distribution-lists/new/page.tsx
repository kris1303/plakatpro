...
function NewDistributionListPageContent() {
  ...
  const calculateDistanceForCity = async (
    cityName: string,
    postalCode: string | null
  ) => {
    ...
    try {
      const destination = postalCode
        ? `${postalCode} ${cityName}, Deutschland`
  ...
  const addItem = async () => {
    ...
    if (formData.eventAddress && calculatedDistance === 0) {
      calculatedDistance = await calculateDistanceForCity(
        city.name,
        city.postalCode
      );
    }
      ...
      <div className="p-8 max-w-6xl">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="card">
            ...
            >
              ➕ Kommune hinzufügen {" "}
              {formData.eventAddress && "(mit Entfernungsberechnung)"}
            </button>
          ...
          <div className="flex gap-3">
            ...
            >
              {loading ? "Speichern..." : "Verteilerliste erstellen"}
            </button>
          </div>
        </form>
      </div>
